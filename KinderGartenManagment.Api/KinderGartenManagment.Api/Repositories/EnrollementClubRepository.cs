...
{
    public class EnrollementClubRepository: IEnrollementClubRepository
    {
        private ApplicationDbContext _context;
        public EnrollementClubRepository(ApplicationDbContext context)
        {
            _context = context;
        }
        public async Task<IEnumerable<Enrollement>> GetAll()
        {
            var t = await _context.Enrollements.Include(c => c.EleveEnrollements).Where(e => e.Type == "Club").ToListAsync();
            return t;
        }
        public async Task<IEnumerable<object>> GetYears(int eleveid)
        {
            return await _context.Enrollements.Where(e => e.Type == "Club" && e.EleveEnrollements.All(ee => ee.EleveId != eleveid)).Select(g => new { debut = g.DateDeDebut.Year, fin = g.DateDeFin.Year }).Distinct().ToListAsync();
        }
        public async Task<IEnumerable<Enrollement>> SearchByYear(int anneededebut, int anneedefin)
        {
            return await _context.Enrollements.Where(e => e.Type == "Club").Where(g => g.DateDeDebut.Year == anneededebut && g.DateDeFin.Year == anneedefin).ToListAsync();
        }
        public async Task<Enrollement> GetByIdAsync(int id)
        {
            return await _context.Enrollements.Where(e => e.Type == "Club").FirstOrDefaultAsync(x => x.Id == id);
        }
