...
{
    public class EnrollementWinterClubRepository : IEnrollementWinterClubRepository
    {
        private ApplicationDbContext _context;
        public EnrollementWinterClubRepository(ApplicationDbContext context)
        {
            _context = context;
        }
        public async Task<IEnumerable<Enrollement>> GetAll()
        {
            var t = await _context.Enrollements.Include(c => c.EleveEnrollements).Where(e => e.Type == "Club d'hiver").ToListAsync();
            return t;
        }
        public async Task<IEnumerable<object>> GetYears(int eleveid)
        {
            return await _context.Enrollements.Where(e => e.Type == "Club d'hiver" && e.EleveEnrollements.All(ee => ee.EleveId != eleveid)).Select(g => new { debut = g.DateDeDebut.Year, fin = g.DateDeFin.Year }).Distinct().ToListAsync();
        }
        public async Task<IEnumerable<Enrollement>> SearchByYear(int anneededebut, int anneedefin)
        {
            return await _context.Enrollements.Where(e => e.Type == "Club d'hiver").Where(g => g.DateDeDebut.Year == anneededebut && g.DateDeFin.Year == anneedefin).ToListAsync();
        }
        public async Task<Enrollement> GetByIdAsync(int id)
        {
            return await _context.Enrollements.Where(e => e.Type == "Club d'hiver").FirstOrDefaultAsync(x => x.Id == id);
        }
        ...
        {
            IEnumerable<Enrollement> Enrollements = _context.Enrollements.Include(e => e.EleveEnrollements).ThenInclude(ee => ee.Eleve).Where(e=>e.Type=="Club d'hiver");
